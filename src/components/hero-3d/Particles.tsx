...
/**
 * Aluminum Studio particle field — cobalt + plata + grafito mix.
 */
export function GoldParticles() {
  ...
  const { positions, velocities, colors, sizes } = useMemo(() => {
    ...
    const cobaltGlow = new THREE.Color("#A8DADC");
    const cobaltDeep = new THREE.Color("#7DBCBE");
    const silver = new THREE.Color("#86868B");
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // 50% cobalt glow, 30% cobalt deep, 20% silver — feel metálico
      const colorPick = Math.random();
      const color = colorPick < 0.5 ? cobaltGlow : colorPick < 0.8 ? cobaltDeep : silver;
      colors[i3] = color.r;
      colors[i3 + 1] = color.g;
      colors[i3 + 2] = color.b;
