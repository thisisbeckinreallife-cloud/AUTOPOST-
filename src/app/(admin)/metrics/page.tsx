  ...
  return (
    <div className="space-y-7 max-w-4xl animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink-9 tracking-tight">Métricas</h1>
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1 bg-ink-2 border border-ink-4 rounded-lg p-1" role="group" aria-label="Rango temporal">
      ...
      {/* Daily chart */}
      <div className="rounded-xl border border-ink-4 bg-ink-2 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-ink-9">
            Posts publicados por día
          </h2>
          <p className="text-xs text-ink-6 tabular-nums">
            Max diario: <span className="font-mono font-semibold text-ink-9">{m.maxDay}</span>
          </p>
        </div>
                ...
                <div
                  className={`w-full rounded-sm transition-all ${
                    !hasData
                      ...
                      : "bg-success group-hover:bg-success"
                  }`}
        ...
        <div className="flex justify-between mt-2 text-[10px] text-ink-6 font-mono tabular-nums">
      ...
      {/* Type breakdown */}
      <div className="rounded-xl border border-ink-4 bg-ink-2 p-5">
        <h2 className="text-sm font-semibold text-ink-9 mb-4">
          Por tipo de contenido
        </h2>
        {m.typeTotal === 0 ? (
          <p className="text-sm text-ink-6 text-center py-4">Sin publicaciones en este rango.</p>
        ) : (
      ...
      {m.businessStats.length > 0 && (
        <div className="rounded-xl border border-ink-4 bg-ink-2 overflow-hidden">
          <div className="px-5 py-4 border-b border-ink-4 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-ink-9">Por cuenta</h2>
            <p className="text-[11px] text-ink-6">Click columna para ordenar</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[640px]">
              <thead>
                <tr className="border-b border-ink-3 text-[10px] font-bold text-ink-7 uppercase tracking-wider">
              ...
              <tbody>
                {sorted.map((biz) => (
                  <tr key={biz.id} className="border-b border-ink-3 last:border-0 hover:bg-ink-2">
                    <td className="px-4 py-3">
                      <Link
                        href={`/businesses/${biz.slug}/chat`}
                        className="font-semibold text-ink-9 hover:underline"
                      >
                        {biz.name}
                      </Link>
                      {biz.igUsername ? (
                        <p className="text-[11px] text-ink-6 flex items-center gap-1 mt-0.5">
                          <Instagram className="h-2.5 w-2.5" />@{biz.igUsername}
                        </p>
                      ) : (
                        <p className="text-[11px] text-warning mt-0.5 font-medium">Sin conectar</p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      <span className="font-bold text-success">{biz.published}</span>
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      <span className={`font-bold ${biz.failed > 0 ? "text-error" : "text-ink-6"}`}>
                        {biz.failed}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {biz.rate !== null ? (
                        <span className="font-bold text-ink-9">{biz.rate}%</span>
                      ) : (
                        <span className="text-ink-6">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums text-xs text-ink-7">
      ...
      {m.businessStats.length === 0 && (
        <div className="text-center py-12 border border-dashed border-ink-4 bg-ink-2 rounded-xl">
          <p className="text-ink-7 text-sm">Todavía no hay datos. Conecta una cuenta y sube tu primer batch.</p>
        </div>
      )}
    </div>
  );
...
function KpiCard({
  ...
  const toneMap = {
    ...
    neutral: { bg: "bg-ink-3",   text: "text-ink-7",   border: "border-ink-4" },
  }[tone];
  ...
  const deltaColor = sign === null ? "text-ink-6" : sign > 0 ? "text-success" : sign < 0 ? "text-error" : "text-ink-6";
  const display = deltaText ?? fmtDelta(delta ?? null);

  return (
    <div className={`rounded-xl border ${toneMap.border} bg-ink-2 p-4 hover:shadow-md transition-shadow`}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs text-ink-7 font-semibold">{label}</span>
      </div>
  ...
  return (
    <div className="relative" style={{ width: 140, height: 140 }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        <span className="font-display font-bold text-2xl text-ink-9 tabular-nums">{total}</span>
        <span className="text-[10px] text-ink-6 uppercase tracking-wider font-mono">posts</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2 w-32 shrink-0">
        ...
        <span className="text-sm text-ink-8 font-medium">{label}</span>
      </div>
      ...
      <span className="text-sm font-semibold text-ink-9 tabular-nums w-12 text-right">
        {value}
      </span>
      <span className="text-xs text-ink-6 tabular-nums w-10 text-right">{pct}%</span>
    </div>
  );
}
  ...
  return (
    <th className={`px-4 py-3 ${numeric ? "text-right" : "text-left"}`}>
      <Link
        ...
        className={`inline-flex items-center gap-1 hover:text-ink-9 transition-colors ${
          isActive ? "text-ink-9" : ""
        }`}
      >
        {children}
        {isActive && <span className="text-ink-6">↓</span>}
      </Link>
    </th>
  );
}
