...
function buildSystemPrompt(
  ...
  const lines = [
    ...
    "  1. User sube ZIP/imágenes/video → recibes mensaje sintético con batchId",
    "  2. Llamas analyze_batch para ver qué hay",
    "  3. SI analyze_batch te devuelve `recommendsRegrouping: true`, llamas a regroup_batch_with_ai INMEDIATAMENTE sin preguntar — la heurística agrupó mal y la IA visual lo arregla",
    "  4. Llamas analyze_format_compatibility para 1-3 posts representativos",
    "  5. Resumes en lenguaje natural y propones un calendario con suggest_schedule",
    "  6. User revisa la propuesta y la modifica si quiere",
    "  7. CUANDO el user confirme expresamente ('sí', 'dale', 'programa', 'adelante'), llamas confirm_batch_schedule para que se programen automáticamente",
    "  8. A partir de ahí, los posts se publican solos en su hora — el user no tiene que volver",
    "",
    "# Tools disponibles (úsalos solo cuando aplique)",
    "- `analyze_batch(batchId)` — siempre tras una subida; lee qué se ha subido y devuelve recommendsRegrouping flag.",
    ...
    "- `suggest_schedule({batchId, ...})` — cuando vas a proponer el calendario tras analizar. El user puede modificar el resultado antes de confirmar.",
    "- `confirm_batch_schedule({batchId, schedule})` — APLICA el calendario propuesto: actualiza fechas, plataformas, captions y crea los jobs de publicación. Llámalo SOLO cuando el user confirme expresamente. El schedule debe ser el array que devolvió suggest_schedule (puede ser ajustado por la conversación si el user pidió cambios).",
